...
import BodyMap from '@/components/CustomForYou/BodyMap';
import { type LanguageCode } from '@/components/CustomForYou/types';
// =============================================
...
interface BookingConfigProps {
  ...
  onConfirm: (data: { bodyParts: { focus: string[], avoid: string[] }; totalDuration: number; timeSlot: string | null; totalPrice: number; appointmentDate: string | null; customerNotes?: string }) => void;
}
...
const BookingConfig = ({ lang, isBookingFlow, selectedStaffIds, selectedStaffInfoList, vipPricingTable, bufferMinutes = 30, onConfirm }: BookingConfigProps) => {
  ...
  const [focusParts, setFocusParts] = useState<string[]>([]);
  const [avoidParts, setAvoidParts] = useState<string[]>([]);
  ...
  const minDuration = 60; // Therapy menu mặc định min 60
  ...
  const handleBodyToggle = (type: 'focus' | 'avoid', area: string) => {
    ...
    if (area === 'FULL_BODY' && type === 'focus') {
        setFocusParts(['HEAD', 'NECK', 'SHOULDER', 'ARM', 'BACK', 'THIGH', 'CALF', 'FOOT']);
        setAvoidParts([]);
        return;
    }

    if (type === 'focus') {
        setFocusParts(prev => prev.includes(area) ? prev.filter(k => k !== area) : [...prev, area]);
        setAvoidParts(prev => prev.filter(k => k !== area));
    } else {
        setAvoidParts(prev => prev.includes(area) ? prev.filter(k => k !== area) : [...prev, area]);
        setFocusParts(prev => prev.filter(k => k !== area));
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Không hiển thị Skills cho Therapy, thay vào đó đi thẳng xuống thời gian và Body Map */}
      ...
      <AnimatePresence>
        {availableDurations.length > 0 && (
          <motion.section
            ...
            className="mb-5 overflow-hidden mt-4"
          >
        ...
        {selectedDuration && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3 mt-3">
            ...
            <section className="p-4 rounded-2xl bg-[#1b1b1d] border border-[#4d463a]/30">
              ...
              <div className="bg-[#0d0d0d] p-3 rounded-xl border border-white/5">
                <BodyMap 
                    focus={focusParts}
                    avoid={avoidParts}
                    lang={lang as LanguageCode}
                    serviceData={{ FOCUS_POSITION: { HEAD: true, NECK: true, SHOULDER: true, ARM: true, BACK: true, THIGH: true, CALF: true, FOOT: true } } as any}
