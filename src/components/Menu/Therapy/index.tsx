...
const TherapyMenu = ({ lang, isBookingFlow, onBack, onCheckout, onSwitchToStandard }: PremiumMenuProps) => {
    const t = getT(lang);
    ...
    const handleBookingConfirm = (data: {
        ...
        const avoidStr = data.bodyParts.avoid.length > 0 ? `Tránh: ${data.bodyParts.avoid.join(', ')}` : '';
        const bodyMapNotes = [focusStr, avoidStr].filter(Boolean).join(' | ');
        
        let finalNotes = data.customerNotes || '';
        if (bodyMapNotes) {
            finalNotes = finalNotes ? `${bodyMapNotes}\n${finalNotes}` : bodyMapNotes;
        }
        ...
        if (isSeparate && selectedStaffIds.length > 1) {
            selectedStaffIds.forEach((staffId) => {
                ...
                addVipToCart({
                    ...
                    customerNotes: finalNotes ? `${finalNotes}\n(Mỗi khách 1 KTV)` : '(Mỗi khách 1 KTV)',
                });
            });
        } else {
            addVipToCart({
