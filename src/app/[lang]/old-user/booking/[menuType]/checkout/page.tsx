...
export default function OldUserBookingCheckoutPage({ params }: { params: Promise<{ lang: string; menuType: string }> }) {
    ...
    const isFakeEmail = (email: string) => /^guest-\d+@no-email\.com$/i.test(email);

    // 🔑 AUTO-FILL: Lấy lại thông tin khách cũ từ Auth Store (Supabase session) hoặc LocalStorage
    useEffect(() => {
        let autoName = '';
        let autoEmail = '';
        let autoPhone = '';
        ...
        // Fallback to localStorage if manual input was used in LoginGate
        if (typeof window !== 'undefined') {
            try {
                const storedInfo = localStorage.getItem('currentUserInfo');
                if (storedInfo) {
                    const parsed = JSON.parse(storedInfo);
                    // LoginGate saves as { fullName, email, phone }
                ...
                console.error("Error parsing currentUserInfo", e);
            }
        }

        // Filter out fake emails before auto-filling
        if (autoEmail && isFakeEmail(autoEmail)) autoEmail = '';
    ...
    const handleBack = () => {
        ...
        router.replace(`/${activeLang}/old-user/booking/${menuType}/menu`);
    };
