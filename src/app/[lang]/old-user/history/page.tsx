    ...
    const handleConfirmAction = (source: 'walk-in' | 'booking') => {
        ...
        setTimeout(() => {
            if (action === 'new') {
                ...
                if (source === 'walk-in') {
                    router.push(`/${lang}/${menuType}/menu?cart=open`);
                } else {
                    router.push(`/${lang}/booking/${menuType}/menu?cart=open`);
                }
                    ...
                    <>
                        {orders.slice(0, visibleCount).map((visit) => (
                            ...
                            <div className="space-y-3 mb-4 mt-3">
                                {visit.items.map((item: any, idx: number) => (
                                    <div key={idx} className="bg-white/[0.03] rounded-xl px-3 py-2.5">
                                        ...
                                        {!(item.ktvRatings && item.ktvRatings.length > 0) && item.staffNames && item.staffNames.length > 0 && (
                                        ...
                                        {!(item.ktvRatings && item.ktvRatings.length > 0) && item.itemRating && item.itemRating > 0 && (
                            ...
                            <div className="flex gap-3 mt-4">
                                {(visit.status === 'DONE' || visit.rating > 0 || visit.items.some((i: any) => (i.ktvRatings && i.ktvRatings.length > 0) || (i.itemRating && i.itemRating > 0))) ? (
                                    <>
                                        <button
